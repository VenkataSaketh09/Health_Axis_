...
const TopDoctors = () => {
  ...
  return (
    <div className="flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10">
      <h1 className="text-3xl font-bold">Top <span className="text-blue-600">Doctors</span> to Book</h1>
      <p className="sm:w-2/3 text-center text-base">Simply browse through our extensive list of trusted doctors.</p>
      <div className="w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0">
        {
            doctors.slice(0,10).map((value,index)=>(
                <div key={index} onClick={()=>navigate(`/appointment/${value._id}`)} className="border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500">
